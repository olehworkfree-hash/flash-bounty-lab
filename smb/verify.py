...
SOURCE = Path(__file__).parent / "scrapers/csi-driver-smb.py"
...
rows = smb.extract_versions(markdown.decode("utf-8"), smb.load_chart_index(index_bytes.decode("utf-8")), ceiling)
...
# Exercise malformed raw YAML at the actual scrape entry point, not only the
# standalone loader. Failure must report an error and preserve all catalog bytes.
controls = {
    "duplicate_top_level": "entries: {}\n" + index_bytes.decode("utf-8"),
    "duplicate_nested": "entries:\n  csi-driver-smb:\n    - name: csi-driver-smb\n      appVersion: 1.20.3\n      appVersion: 1.19.1\n      version: 1.20.3\n",
    "alias": "shared: &shared {}\nentries: *shared\n",
    "merge_key": "entries: {csi-driver-smb: [{<<: {name: csi-driver-smb}}]}\n",
}
control_results = {}
for name, raw in controls.items():
    utils.fetch_page = lambda url, data=raw.encode("utf-8"): data if url == smb.INDEX_URL else real_fetch(url)
    before_errors = len(errors)
    before = target.read_bytes()
    smb.scrape()
    assert len(errors) == before_errors + 1, name
    assert target.read_bytes() == before == first, name
    control_results[name] = "rejected_file_unchanged"
utils.fetch_page = real_fetch
shutil.copyfile(target, OUT / "csi-driver-smb.generated.yaml")
report = {
    "upstream_commit": PIN,
    "scraper_sha256": hashlib.sha256(SOURCE.read_bytes()).hexdigest(),
    "strict_yaml_controls": control_results,
