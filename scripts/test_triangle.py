...
def fixture():
    ...
    pools=mock_pools();r=build_routes(pools)[0];n=int(r['amount_in']);fee=premium(n,5)
    ...
    s=dict(schema='flash.triangle_screen.v1',status='PASS',execution_allowed=False,realized_pnl='0',header=h,block_number=100,
        quorum_sha256=q['evidence_sha256'],voters=q['state_voters'],observations=dict(pools=pools,rows=[r],premium_bps=5),
        requested=1,quoted=1,positive_before_gas=0,matched_fork_selection=[r])
    s['sha256']=digest(s)
    return q,s
