...
def main():
    ...
    report=dict(schema='flash.v3_fork_quote_match.v1',status='FAILED',realized_pnl='0',mainnet_broadcast=False,user_funds_used=False,exact_arbitrum_fee_known=False,local_native_eth_seeded=True,flash_loan_used_in_this_test=False,source_commit=os.getenv('GITHUB_SHA'))
    proc=None
    try:
        ...
        for label in ['alchemy-public','publicnode','arbitrum-official']:
            ...
            with (root/f'anvil-v3-{label}.log').open('w') as log:
        ...
        with (root/'v3-fork-tests.log').open('w') as log:
            run=subprocess.run(['forge','test','--fork-url',LOCAL,'--match-contract','V3QuoteExecutionForkTest','-vvvv'],env=env,stdout=log,stderr=subprocess.STDOUT,timeout=420)
        if run.returncode: raise ValueError('V3_FORGE_TEST_FAILED')
    ...
    report['sha256']=digest(report); (root/'v3-fork-gate.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report))
